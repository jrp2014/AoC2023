...
mkDiagram n e = undir $ mkGraph (map swap n') e'
  where
    n' = zip n [1 ..]
    e' = map (\(from, to) -> (fromJust $ lookup from n', fromJust $ lookup to n', 1)) e
...
flows :: Diagram -> [(Node, Node, Int)]
--flows diagram =  takeWhile (\(x, y) -> (maxFlow diagram x y) /= 3 )  [(x,  y) | x <- nodes diagram, y<- nodes diagram, x /= y]
flows diagram =  [(x,  y, maxFlow diagram x y) | x <- nodes diagram, y<- nodes diagram, x /= y]
